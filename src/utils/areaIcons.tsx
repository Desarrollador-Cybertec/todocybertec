...
/** Renders the icon for an area using the key stored in area.icon_key */
...
export function getAreaIconKey(areaId: number): string {
  return readStorage()[String(areaId)] ?? 'office';
}

export function setAreaIconKey(areaId: number, key: string): void {
  const stored = readStorage();
  stored[String(areaId)] = key;
  localStorage.setItem(STORAGE_KEY, JSON.stringify(stored));
}

export function AreaIconDisplay({ areaId, className = 'h-5 w-5' }: { areaId: number; className?: string }) {
  const key = getAreaIconKey(areaId);
  const def = AREA_ICONS.find((i) => i.key === key) ?? AREA_ICONS[0];
  const Icon = def.icon;
  return <Icon className={className} />;
}
