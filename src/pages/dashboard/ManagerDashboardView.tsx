    ...
    <div id="dashboard-content" className="max-w-full overflow-hidden space-y-6">
      ...
      <FadeIn className="flex flex-wrap items-center justify-between gap-3 sm:gap-4 rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito px-4 py-4 sm:px-6 sm:py-5 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Tu área tiene <span className="font-semibold text-slate-900 dark:text-white">{areaData?.total_tasks ?? 0} tareas</span>
            {' '}y tienes <span className="font-semibold text-slate-900 dark:text-white">{personalTasks.length} tareas propias activas</span>
            {attentionCount > 0 && (
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6 lg:col-span-2 lg:order-1">
          ...
          <FadeIn delay={0.2} id="dashboard-stats" className="rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito p-5 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <MiniStat label="Total"       value={areaData?.total_tasks ?? 0}     icon={<HiOutlineClipboardList className="h-5 w-5" />} color="text-cyber-radar dark:text-cyber-radar-light bg-cyber-radar/10"     to={areaData?.area?.id ? `/tasks?area_id=${areaData.area.id}&type=org` : '/tasks'} />
              <MiniStat label="Vencidas"    value={areaData?.overdue_tasks ?? 0}   icon={<HiOutlineExclamation className="h-5 w-5" />}   color="text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30"         alert={(areaData?.overdue_tasks ?? 0) > 0} to={areaData?.area?.id ? `/tasks?status=overdue&area_id=${areaData.area.id}&type=org` : '/tasks?status=overdue'} />
              <MiniStat label="Sin progreso" value={areaData?.without_progress ?? 0} icon={<HiOutlineClock className="h-5 w-5" />}       color="text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30"  alert={(areaData?.without_progress ?? 0) > 0} to={areaData?.area?.id ? `/tasks?status=pending&area_id=${areaData.area.id}&type=org` : '/tasks?status=pending'} />
              <MiniStat label="Completadas" value={areaData?.completed_tasks ?? 0} icon={<HiOutlineCheckCircle className="h-5 w-5" />}   color="text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30"  to={areaData?.area?.id ? `/tasks?status=completed&area_id=${areaData.area.id}&type=org` : '/tasks?status=completed'} />
            </div>
            {areaData?.completion_rate != null && (
              <div className="mt-4">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500 dark:text-slate-400">Tasa de cumplimiento</span>
                ...
                <div className="mt-1.5 h-2 w-full rounded-full bg-slate-100 dark:bg-white/10">
                  <div
          ...
          <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div className="divide-y divide-slate-50 dark:divide-white/5 px-5">
                {!areaData?.by_responsible?.length ? (
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    <HiOutlineUserGroup className="mb-2 h-8 w-8 text-slate-300 dark:text-slate-500" />
                    <p className="text-sm text-slate-500 dark:text-slate-400">Sin datos de carga</p>
                  </div>
                ) : (
            ...
            {/* Por estado del área */}
            <FadeIn delay={0.3} className="rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito shadow-sm">
              <div className="border-b border-slate-200 dark:border-white/5 px-5 py-4">
                <h3 className="font-semibold text-slate-900 dark:text-white">Por estado (área)</h3>
              </div>
              <div className="divide-y divide-slate-50 dark:divide-white/5 px-5">
                {(() => {
                  ...
                  if (entries.length === 0) {
                    return <p className="py-6 text-center text-sm text-slate-400 dark:text-slate-500">Sin datos de estado</p>;
                  }
                      ...
                      <div key={status} className="flex items-center gap-3 py-2.5">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 h-1.5 w-full rounded-full bg-slate-100 dark:bg-white/10">
                            <div
                              className={`h-1.5 rounded-full transition-all ${status === 'completed' ? 'bg-green-500' : status === 'overdue' ? 'bg-red-500' : 'bg-cyber-radar'}`}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        </div>
        ...
        <div className="flex flex-col gap-6 lg:order-2">
            ...
            <div className="divide-y divide-slate-50 dark:divide-white/5 px-5">
              {urgentTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-sm font-medium text-slate-600 dark:text-slate-400">¡Todo bajo control!</p>
                </div>
              ) : (
          ...
          <FadeIn delay={0.15} className="rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/5 px-5 py-4">
              <h3 className="font-semibold text-slate-900 dark:text-white">Mis próximas tareas</h3>
              <Link to="/tasks" className="rounded-lg bg-white dark:bg-cyber-grafito border border-slate-200 dark:border-white/10 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 transition-colors hover:bg-slate-50 dark:hover:bg-white/5">
                Ver todas
              </Link>
            </div>
            <div className="divide-y divide-slate-50 dark:divide-white/5">
              {allTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center px-5 py-8 text-center">
                  <HiOutlineClipboardList className="mb-2 h-8 w-8 text-slate-300 dark:text-slate-500" />
                  <p className="text-sm text-slate-500 dark:text-slate-400">Sin tareas asignadas</p>
                </div>
              ) : (
