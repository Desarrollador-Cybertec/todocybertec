...
export function SuperAdminDashboard() {
  ...
  if (error || !data) return <p className="text-slate-500 dark:text-slate-400">No se pudo cargar el dashboard general.</p>;
      ...
      <FadeIn className="flex flex-wrap items-center justify-between gap-3 sm:gap-4 rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito px-4 py-4 sm:px-6 sm:py-5 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Hay <span className="font-semibold text-slate-900 dark:text-white">{data.total_active} tareas activas</span> en la organización
        ...
        <FadeIn delay={0.1} className="lg:col-span-2 rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito shadow-sm">
          ...
          <div className="divide-y divide-slate-50 dark:divide-white/5 px-4 sm:px-6">
            {(() => {
              ...
              if (entries.length === 0) {
                return <p className="py-6 text-center text-sm text-slate-400 dark:text-slate-500">Sin datos de estado disponibles</p>;
              }
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <FadeIn delay={0.15} className="lg:col-span-3 rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/5 px-4 py-4 sm:px-6">
            ...
            <Link to="/users" className="rounded-lg bg-white dark:bg-cyber-grafito border border-slate-200 dark:border-white/10 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 transition-colors hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white">
              Ver usuarios
            </Link>
          </div>
          <div className="divide-y divide-slate-50 dark:divide-white/5 px-4 sm:px-6">
            {topOverloaded.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-sm font-medium text-slate-600 dark:text-slate-400">Sin tareas pendientes</p>
              </div>
            ) : (
              topOverloaded.map((r: PendingByUser) => {
                ...
                return (
                  <div key={r.user_id} className="flex items-center gap-3 py-3.5">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-cyber-radar text-sm font-medium text-white">
                      {r.user_name.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <p className="truncate text-sm font-medium text-slate-900 dark:text-white">{r.user_name}</p>
                        <span className={`text-sm font-bold ${r.pending_tasks > 5 ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-white'}`}>{r.pending_tasks}</span>
                      </div>
                      <div className="mt-1.5 h-1.5 w-full rounded-full bg-slate-100 dark:bg-white/10">
                        <div className={`h-1.5 rounded-full transition-all ${r.pending_tasks > 5 ? 'bg-red-400' : 'bg-cyber-radar'}`} style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  </div>
                );
          ...
          <FadeIn delay={0.2} className="rounded-sm border border-slate-200 dark:border-white/5 bg-white dark:bg-cyber-grafito shadow-sm">
            ...
            <div className="divide-y divide-slate-50 dark:divide-white/5 px-5">
              {(data.tasks_by_area ?? []).length === 0 ? (
                <p className="py-6 text-center text-xs text-slate-400 dark:text-slate-500">Sin áreas registradas</p>
              ) : (
                (data.tasks_by_area ?? []).map((a) => (
                  <Link key={a.area_id} to={`/areas/${a.area_id}`} className="group flex items-center justify-between py-3 transition-colors hover:text-cyber-radar">
                    <span className="text-sm text-slate-700 dark:text-slate-300 group-hover:text-cyber-radar dark:group-hover:text-cyber-radar-light">{a.area_name}</span>
            ...
            <div className="divide-y divide-slate-50 dark:divide-white/5">
              {!data.my_tasks || data.my_tasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs font-medium text-slate-600 dark:text-slate-400">Sin tareas asignadas</p>
                </div>
              ) : (
                data.my_tasks.map((t) => (
          ...
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100 dark:bg-white/10">
            <div
              className={`h-1.5 rounded-full transition-all ${
                task.progress_percent >= 100 ? 'bg-green-500' : task.is_overdue ? 'bg-red-400' : 'bg-cyber-radar'
              }`}
              style={{ width: `${Math.min(task.progress_percent, 100)}%` }}
            />
          </div>
          <span className="shrink-0 text-xs text-slate-500 dark:text-slate-400">{task.progress_percent}%</span>
        </div>
